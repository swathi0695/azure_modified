...
class AzureBaseAction(Action):
	...
	def get_credentials(self):
		...
		with open("/opt/stackstorm/packs/azure_modified/actions/lib/source_tokenizer_config.yaml", "rb") as config_file:
			azure_config = yaml.load(config_file.read(), Loader=yaml.UnsafeLoader)
			subscription_id = azure_config['compute']['subscription_id']
			credentials = ServicePrincipalCredentials(
				client_id=azure_config['resource_manager']['client_id'],
				secret=azure_config['resource_manager']['secret'],
				tenant=azure_config['resource_manager']['tenant']
			)
		return credentials, subscription_id
